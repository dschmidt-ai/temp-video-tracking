...
def make_parser():
    ...
    :return:
    """
    parser = argparse.ArgumentParser("tracking main entrypoint!")
    ...
    parser.add_argument("--n_classes",
                        type=int,
                        default=5,
                        help="number of model classes for associated model")  # number of object classes
    parser.add_argument("--class_names",
                        type=str,
                        default="car, bicycle, person, cyclist, tricycle",
                        help="class names for associated model")
    parser.add_argument("--class_names_filter",
                        type=str,
                        default=None,
                        help="relevant classes for output (must be subset of --class_names)")
    ...
    while True:
        ...
        if ret_val:
            ...
            if opt.save_result:
                ...
                write_results_mcmot(f_path=vid_save_path.split('.')[0]+'.txt',
                                    frame_id=frame_id,
...
def imageflow_demo(predictor, vis_dir, current_time, opt):
    ...
    if opt.demo == "videos":
        ...
        if os.path.isfile(opt.path):
            ...
            logger.info('Save results to {}'.format(opt.path.split('.')[0]+'.txt'))
...
if __name__ == "__main__":
    ...
    # filter classes if class filter is applied
    if opt.class_names_filter is not None:
        filtered_class_names = opt.class_names_filter.split(",")
    else:
        filtered_class_names = opt.class_names

    opt.class_names_filter = filtered_class_names
    opt.n_classes_filter = len(filtered_class_names)

    exp.class_names = class_names
    exp.n_classes = len(exp.class_names)
    exp.class_names_filter = filtered_class_names
    exp.n_classes_filter = len(exp.class_names_filter)

    print("Number of detector classes: ", exp.n_classes)
    print("Number of output classes: ", exp.n_classes_filter)
